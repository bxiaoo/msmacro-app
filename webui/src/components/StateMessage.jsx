import { Disc3, Circle, Eye } from "lucide-react";
  ...
  return (
    <div className="relative shrink-0 w-full" data-name="state msg">
      <div className="flex flex-row items-center relative size-full">
        {/* CV-AUTO Playing (Green) - Priority over CV Activated */}
        {isCVPlaying && <div className="bg-emerald-100 box-border content-stretch flex gap-1 items-center justify-start p-3 relative w-full">
          ...
          <div className="basis-0 p-1 content-stretch flex flex-col gap-1 grow items-start justify-start leading-[0] min-h-px min-w-px relative shrink-0 text-nowrap">
            <div className="font-['Roboto:Bold',_sans-serif] font-bold min-w-full overflow-ellipsis overflow-hidden relative shrink-0 text-[0px]" style={{ width: "min-content", fontVariationSettings: "'wdth' 100" }}>
              <p className="[text-overflow:inherit] [text-wrap-mode:inherit] [white-space-collapse:inherit] font-['Roboto:Black',_sans-serif] font-black leading-[normal] overflow-inherit text-[18px]">
                ...
                <span className="text-emerald-700" style={{ fontVariationSettings: "'wdth' 100" }}>{` ${cvItemName}`}</span>
              </p>
            </div>
        ...
        {/* CV Activated (Yellow) */}
        {!isCVPlaying && isCVActivated && <div className="bg-yellow-100 box-border content-stretch flex gap-1 items-center justify-start p-3 relative w-full">
          <div className="relative shrink-0 size-[38px] flex items-center justify-center rounded-full">
            <Eye strokeWidth={1.2} size={44} className="text-yellow-700" />
          </div>
          <div className="basis-0 p-1 content-stretch flex flex-col gap-1 grow items-start justify-start leading-[0] min-h-px min-w-px relative shrink-0 text-nowrap">
            <div className="font-['Roboto:Bold',_sans-serif] font-bold min-w-full overflow-ellipsis overflow-hidden relative shrink-0 text-[0px]" style={{ width: "min-content", fontVariationSettings: "'wdth' 100" }}>
              <p className="[text-overflow:inherit] [text-wrap-mode:inherit] [white-space-collapse:inherit] font-['Roboto:Black',_sans-serif] font-black leading-[normal] overflow-inherit text-[18px]">
                <span className="text-yellow-700" style={{ fontVariationSettings: "'wdth' 100" }}>
                  Selected:
                </span>
                <span className="text-yellow-700" style={{ fontVariationSettings: "'wdth' 100" }}>{` ${cvItemName}`}</span>
              </p>
            </div>
            <div className="font-['Roboto:Medium',_sans-serif] font-medium relative shrink-0 text-[14px] text-yellow-700" style={{ fontVariationSettings: "'wdth' 100" }}>
              <p className="leading-[normal] text-nowrap whitespace-pre">Player current position (32, 40)</p>
            </div>
          </div>
        </div>}
        {/* Regular Rotation Playing (Green) */}
        {!isCVActivated && !isCVPlaying && isPlaying && <div className="bg-emerald-100 box-border content-stretch flex gap-1 items-center justify-start p-3 relative w-full">
          <div className="relative shrink-0 size-[38px] flex items-center justify-center rounded-full">
            <Disc3 strokeWidth={1.2} size={44} className="text-emerald-700 animate-spin" />
          </div>
          <div className="basis-0 p-1 content-stretch flex flex-col gap-1 grow items-start justify-start leading-[0] min-h-px min-w-px relative shrink-0 text-nowrap">
        ...
        {/* Recording (Blue) */}
        {!isCVActivated && !isCVPlaying && isRecording && <div className="bg-blue-100 box-border content-stretch flex gap-2 items-center justify-start p-[8px] relative w-full">
