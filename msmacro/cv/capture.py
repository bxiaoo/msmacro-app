...
class CVCapture:
    ...
    async def start(self) -> None:
        ...
        """
        # Health check: detect zombie state (thread died but _running=True)
        if self._running:
            if self._capture_thread and self._capture_thread.is_alive():
                ...
                logger.warning(
                    "⚠️ ZOMBIE STATE DETECTED: Capture was marked running but thread is dead - "
                    "resetting state and restarting"
                )
                self._running = False
        ...
        try:
            while not self._stop_event.is_set():
                try:
                    ...
                    self._frames_failed += 1
                    logger.error(
                        f"Error in capture loop ({consecutive_errors}/{MAX_CONSECUTIVE_ERRORS}): {e}",
                        exc_info=True
                    )
                    self._set_last_error("Unexpected error in capture loop", exception=e)

                    # Circuit breaker: exit loop if too many consecutive errors
                    if consecutive_errors >= MAX_CONSECUTIVE_ERRORS:
                        logger.critical(
                            f"❌ CIRCUIT BREAKER: {MAX_CONSECUTIVE_ERRORS} consecutive errors - "
                            "exiting capture loop to prevent infinite error loop"
                        )
                        self._set_last_error(
                            f"Capture loop failed with {MAX_CONSECUTIVE_ERRORS} consecutive errors"
                        )
                        break
            ...
            # ALWAYS reset state flags when loop exits (normal stop or crash)
            logger.info("Capture loop stopped - resetting state flags")
            self._running = False
            self._device_connected = False
